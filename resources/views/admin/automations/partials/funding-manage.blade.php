    ...
    <section class="rounded-xl border border-gray-200 p-4 dark:border-gray-700">
        ...
        <form method="POST" action="{{ route('admin.automation-funding.configure', $automation) }}" class="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div class="sm:col-span-2 mt-1 border-t border-gray-200 pt-4 dark:border-gray-700">
                <h5 class="text-xs font-semibold text-gray-900 dark:text-gray-100">Automation provider destination bank</h5>
    ...
    @if($funding)
        <section class="rounded-xl border border-gray-200 p-4 dark:border-gray-700">
            ...
            <div class="mt-4 flex flex-wrap gap-2">
                ...
                <form method="POST" action="{{ route('admin.automation-funding.save-bank-info', $funding) }}">@csrf<button class="ti-btn ti-btn-primary ti-btn-sm" @disabled(!$funding->securewave_customer_created_at)>{{ $funding->securewave_bank_info_saved_at ? 'Update Securewave Bank Info' : 'Save Bank Info on Securewave' }}</button></form>
            ...
            <p class="mt-3 text-xs {{ $funding->securewave_bank_info_saved_at ? 'text-success' : 'text-warning' }}">
                Bank registration: {{ $funding->securewave_bank_info_saved_at ? 'Saved on Securewave' : 'Required before funding' }}
            </p>
        </section>
