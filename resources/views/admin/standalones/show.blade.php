...
<div class="main-content">
    ...
    <div class="box"><div class="box-header"><h5 class="box-title">Available features</h5></div><div class="box-body overflow-auto"><table class="ti-custom-table ti-custom-table-head"><thead><tr><th>Feature</th><th>Billing</th><th>Setup / initial</th><th>Monthly</th><th>Status</th></tr></thead><tbody>@foreach($features as $feature) @php($subscription=$subscriptions->get($feature->id))<tr><td><strong>{{ $feature->name }}</strong></td><td>{{ str_replace('_',' ',ucfirst($feature->billing_type)) }}</td><td>₦{{ number_format((float)$feature->purchasePriceFor($site->price_level),2) }}</td><td>{{ $feature->isRecurring() ? '₦'.number_format((float)$feature->monthlyPriceFor($site->price_level),2) : '—' }}</td><td>{{ ucfirst($subscription?->status ?? ($feature->is_active ? 'Available' : 'Unavailable')) }}</td></tr>@endforeach</tbody></table></div></div>

    <div class="box"><div class="box-header"><h5 class="box-title">Feature purchase history</h5></div><div class="box-body overflow-auto"><table class="ti-custom-table ti-custom-table-head"><thead><tr><th>Date</th><th>Feature</th><th>Event</th><th>Price level</th><th>Amount</th><th>Reference</th></tr></thead><tbody>@forelse($featurePurchases as $purchase)<tr><td>{{ $purchase->created_at?->format('d M Y H:i') }}</td><td>{{ $purchase->feature->name }}</td><td>{{ ucfirst($purchase->billing_event) }}</td><td>{{ str_replace('_',' ',ucfirst($purchase->applied_price_level)) }}</td><td>₦{{ number_format((float)$purchase->amount,2) }}</td><td>{{ $purchase->client_reference }}</td></tr>@empty<tr><td colspan="6" class="py-8 text-center text-gray-500">No feature purchases yet.</td></tr>@endforelse</tbody></table></div>@if($featurePurchases->hasPages())<div class="box-footer">{{ $featurePurchases->links() }}</div>@endif</div>
