...
class FeatureController extends Controller
{
    public function index(Request $request): JsonResponse
    {
        ...
        $subscriptions = $site->featureSubscriptions()->get()->keyBy('standalone_feature_id');
        $features = StandaloneFeature::where('is_active', true)->orderBy('sort_order')->orderBy('name')->get()
            ->map(fn (StandaloneFeature $feature): array => $this->featureData($feature, $site, $subscriptions->get($feature->id)));
    ...
    public function show(Request $request, string $feature): JsonResponse
    {
        ...
        $subscription = $site->featureSubscriptions()->where('standalone_feature_id', $model->id)->first();

        return response()->json(['success' => true, 'data' => $this->featureData($model, $site, $subscription)]);
    }

    public function purchase(Request $request, string $feature, PurchaseStandaloneFeature $purchase): JsonResponse
    {
        $data = $request->validate(['reference' => ['required', 'string', 'max:100', 'regex:/^[A-Za-z0-9._:-]+$/']]);
        $model = StandaloneFeature::where('slug', $feature)->firstOrFail();
        try {
            $result = $purchase->handle($this->site($request), $model, trim($data['reference']));
    ...
    public function purchases(Request $request): JsonResponse
    {
        ...
        return response()->json(['success' => true, 'data' => collect($purchases->items())->map(fn ($purchase) => [
            ...
            'feature' => $purchase->feature->slug, 'billing_event' => $purchase->billing_event,
    ...
    {
        $model = StandaloneFeature::where('slug', $feature)->firstOrFail();
        $subscription = $this->site($request)->featureSubscriptions()->where('standalone_feature_id', $model->id)->firstOrFail();
    ...
    private function featureData(StandaloneFeature $feature, StandaloneWebsite $site, ?StandaloneFeatureSubscription $subscription): array
    {
        return ['slug' => $feature->slug, 'name' => $feature->name, 'description' => $feature->description,
            'billing_type' => $feature->billing_type, 'price' => $feature->purchasePriceFor($site->price_level),
    ...
    private function purchaseData($purchase, StandaloneFeatureSubscription $subscription): array
    {
        return ['transaction_id' => $purchase->transaction_id, 'reference' => $purchase->client_reference,
            'feature' => $purchase->feature->slug, 'billing_event' => $purchase->billing_event, 'amount' => $purchase->amount,
