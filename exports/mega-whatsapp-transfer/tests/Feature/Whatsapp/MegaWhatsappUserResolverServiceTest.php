...
it('normalizes Nigerian international phone numbers', function (string $input, string $expected) {
    expect(app(MegaWhatsappUserResolverService::class)->normalize($input))->toBe($expected);
})->with([
...
it('resolves a user by their registered phone number', function () {
    ...
    $resolved = app(MegaWhatsappUserResolverService::class)->resolve('+2348012345678');
...
it('returns null when a WhatsApp number is not linked', function () {
    expect(app(MegaWhatsappUserResolverService::class)->resolve('2348012345678'))->toBeNull();
});
